...
@Component
public class M86ModelTagsProcessor implements PageProcessor {
                ...
                .forEach(e -> {
                    ...
                    modelLinks.forEach(modelLink -> {
                        ...
                        if (tagText != null && tagHref != null) {
                            ...
                            tagService.createTag(TagType.MODEL, tagText);
    ...
    private void processAlbumPage(Page page, Selectable albumContainer) {
        ...
        long tagId = tagService.createTag(TagType.NORMAL, "模特");
        tagIds.add(tagId);

        final String tagText = tagNavLink.xpath("a/span[@style='color:red']/text()").toString();
        if (tagText != null) {
            tagId = tagService.createTag(TagType.MODEL, tagText);
            tagIds.add(tagId);
        }
    ...
    private void saveAlbum(final Page page, List<Selectable> albumList, List<Long> tagIds) {
